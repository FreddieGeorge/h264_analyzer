...
- `cabac/H264CabacContextModel.*`: CABAC context-model initialization
  tables/helpers. The covered subset currently reaches ctxIdx 248, including
  ...
  first luma4x4 `coeff_abs_level_minus1` context used by the narrow CABAC
  paths.
...
- `cabac/H264CabacResidualSyntaxReader.*`: focused residual CABAC syntax
  ...
  `coeff_abs_level_minus1` prefix bin with ctxIdx 248 before returning
  `cabac_residual_incomplete`. If that first prefix bin is zero, the next
  unsupported stage is `coeff_sign_flag`; if it is one, the next unsupported
  ...
  map. Chroma non-zero CBF, complete significant/last maps, and coefficient
  level/sign parsing are not implemented.
...
- CABAC macroblock model parsing is not implemented. Groundwork includes
  ...
  initialization currently covers ctxIdx 0-248, including B-slice skip/type
  ...
  `last_significant_coeff_flag` contexts plus the first luma4x4
  `coeff_abs_level_minus1` context.
  ...
  prefix bins, incomplete block, incomplete scan index, category, and next
  unsupported stage on the syntax result before returning
...
1. Broaden the newly wired residual-CABAC path one step at a time. Current
   ...
   `coeff_abs_level_minus1` prefix bin,
...
4. Next residual step should likely either broaden significant/last scan
   coverage beyond the first four luma4x4 positions or add the next
   `coeff_abs_level_minus1` prefix-bin step. Do not jump directly to full
   coefficient levels and signs.
