#include "core/analysis/AnalysisStats.h"

#include <QtGlobal>

#include <cmath>

namespace
{
void require(bool condition, const char *message)
{
    if (!condition) {
        qFatal("%s", message);
    }
}
}

int main()
{
    FrameAnalysis video;
    video.frameIndex = 0;
    video.mediaKind = MediaKind::Video;
    video.accessUnitKind = AccessUnitKind::VideoFrame;
    video.frameType = QStringLiteral("P");
    video.hasFrame = true;

    AnalysisRegion parsedMb;
    parsedMb.kind = AnalysisRegionKind::Macroblock;
    parsedMb.parsed = true;
    parsedMb.qp = 22;
    video.regions.append(parsedMb);

    AnalysisRegion skippedMb;
    skippedMb.kind = AnalysisRegionKind::Macroblock;
    skippedMb.skipped = true;
    skippedMb.qp = 26;
    video.regions.append(skippedMb);

    AnalysisMotionVector mv;
    mv.mvXQuarterPel = 3;
    mv.mvYQuarterPel = 4;
    video.motionVectors.append(mv);

    AnalysisDiagnostic diagnostic;
    diagnostic.code = QStringLiteral("slice.short");
    diagnostic.severity = QStringLiteral("warning");
    video.diagnostics.append(diagnostic);

    FrameAnalysis audio;
    audio.frameIndex = 1;
    audio.mediaKind = MediaKind::Audio;
    audio.accessUnitKind = AccessUnitKind::AudioFrame;

    const AnalysisStats stats = calculateAnalysisStats({video, audio});

    require(stats.totalAccessUnits == 2, "total access unit count mismatch");
    require(stats.videoAccessUnits == 1, "video access unit count mismatch");
    require(stats.audioAccessUnits == 1, "audio access unit count mismatch");
    require(stats.frameCount == 1, "frame count mismatch");
    require(stats.macroblockRegionCount == 2, "macroblock count mismatch");
    require(stats.parsedMacroblockRegionCount == 1, "parsed macroblock count mismatch");
    require(stats.skippedMacroblockRegionCount == 1, "skipped macroblock count mismatch");
    require(stats.qpValueCount == 2, "qp value count mismatch");
    require(stats.minQp == 22, "min qp mismatch");
    require(stats.maxQp == 26, "max qp mismatch");
    require(std::abs(stats.averageQp - 24.0) < 0.001, "average qp mismatch");
    require(stats.motionVectorCount == 1, "motion vector count mismatch");
    require(std::abs(stats.averageMvMagnitudeQuarterPel - 5.0) < 0.001, "average mv magnitude mismatch");
    require(stats.maxMvMagnitudeQuarterPel == 5, "max mv magnitude mismatch");
    require(stats.diagnosticAccessUnits == 1, "diagnostic access unit count mismatch");
    require(stats.diagnosticCount == 1, "diagnostic count mismatch");
    require(stats.diagnostics.size() == 1, "diagnostic summary count mismatch");
    require(stats.diagnostics.first().code == QStringLiteral("slice.short"), "diagnostic code mismatch");
    require(stats.frameTypes.size() == 1, "frame type summary count mismatch");
    require(stats.frameTypes.first().type == QStringLiteral("P"), "frame type mismatch");
    require(stats.frameTypes.first().count == 1, "frame type count mismatch");

    return 0;
}
